...
public class ExampleShipControl : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			GetComponent<Rigidbody2D>().AddForce(acceleration_amount * Time.deltaTime * transform.up);
		...
		{
			GetComponent<Rigidbody2D>().AddForce(0.6f * acceleration_amount * Time.deltaTime * (-transform.right));
			//print ("strafeing");
		}
